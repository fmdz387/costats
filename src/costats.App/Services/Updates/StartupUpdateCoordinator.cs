...
if (Get-Process -Id $TargetPid -ErrorAction SilentlyContinue) {
    Write-Log "Target process still running after timeout. Stopping forcefully."
...
if (-not (Test-Path $newExePath)) {
    throw "Updated executable not found: $newExePath"
}

try {
    Start-Process -FilePath $newExePath | Out-Null
    Write-Log "Launched updated app: $newExePath"
} catch {
    Write-Log "Failed to relaunch app: $($_.Exception.Message)"
    throw
}
...
if (Test-Path $backupDir) {
    try {
        ...
        Write-Log "Backup cleanup failed: $($_.Exception.Message)"
    }
}
